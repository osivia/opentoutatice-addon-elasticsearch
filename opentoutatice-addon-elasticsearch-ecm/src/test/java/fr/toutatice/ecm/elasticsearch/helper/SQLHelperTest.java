...
public class SQLHelperTest extends TestCase {
    ...
    public void testEscape10() {
        assertEquals("SELECT * FROM Document WHERE dc:title LIKE 'l\\'arche' AND dc:subjects IN ('l\\'olivier','le poirier AND l\\'abricotier')", SQLHelper
                .getInstance().escape("SELECT * FROM Document WHERE dc:title LIKE 'l'arche' AND dc:subjects IN ('l'olivier','le poirier AND l'abricotier')"));
    }

    // multiple predicates (global search like)
    public void testEscape11() {
        assertEquals(
                "SELECT * FROM Document WHERE ecm:fulltext = 'Espace d\\'apprentissage en scolarité -noindex' AND (ecm:isProxy = 1 AND ecm:mixinType != 'HiddenInNavigation'  AND ecm:currentLifeCycleState <> 'deleted' )",
                SQLHelper.getInstance().escape(
                        "SELECT * FROM Document WHERE ecm:fulltext = 'Espace d'apprentissage en scolarité -noindex' AND (ecm:isProxy = 1 AND ecm:mixinType != 'HiddenInNavigation'  AND ecm:currentLifeCycleState <> 'deleted' )"));
    }
    ...
    public void testEscape16() {
        assertEquals(
                "SELECT * FROM Document WHERE (ecm:primaryType in ('Note','Article','File','Picture','Audio','Video')) AND ((dc:issued between DATE '2016-07-11' AND DATE '2016-07-12'))",
                SQLHelper.getInstance().escape(
                        "SELECT * FROM Document WHERE (ecm:primaryType in ('Note','Article','File','Picture','Audio','Video')) AND ((dc:issued between DATE '2016-07-11' AND DATE '2016-07-12'))"));
    }
    ...
    public void testEscape17() {
        assertEquals(
                "SELECT * FROM Document WHERE dc:title LIKE 'l\\'arche%' and (ecm:primaryType in ('Note','Article','File','Picture','Audio','Video')) AND ecm:path STARTSWITH 'ah - la' AND ((dc:issued BETWEEN DATE \'2016-07-11\' AND DATE \'2016-07-12')) AND ((ecm:primaryType not in ('BulkDocumentRoot'))) "
